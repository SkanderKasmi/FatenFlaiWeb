...
function getTextPartsArray(el, class) {
  ...
  if (len) {
    ...
    if (remainingMarkup !== '') {
      const unTaggedRemaining = wrapContentsSpan(remainingMarkup, class);
...
function setSegments(target, newText) {
  ...
  target.innerHTML = '';
  target.innerHTML += oldTargetSegs.map((s) => { s.class += ' oldText'; return s.outerHTML; }).join('');
  target.innerHTML += newTargetSegs.map((s) => { s.class += ' newText'; return s.outerHTML.replace(s.innerHTML, ''); }).join('');
  /* eslint-enable no-param-reassign */
...
function createTextTweens(target, newText, ops) {
  ...
  const oldTargets = [].slice.call(target.getElementsByclass('oldText')).reverse();
  const newTargets = [].slice.call(target.getElementsByclass('newText'));
