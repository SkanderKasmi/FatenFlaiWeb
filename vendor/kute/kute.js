...
(function (global, factory) {
  ...
  function wrapContentsSpan(el, class) {
    var assign;
    ...
    if (!el.children.length || (el.children.length && el.children[0].class !== class)) {
      var elementInnerHTML = el.innerHTML;
      textWriteWrapper = document.createElement('SPAN');
      textWriteWrapper.class = class;
    ...
    } else if (el.children.length && el.children[0].class === class) {
      (assign = el.children, textWriteWrapper = assign[0]);
    }
    return textWriteWrapper;
  }

  function getTextPartsArray(el, class) {
    var elementsArray = [];
  ...
  function setSegments(target, newText) {
    ...
    target.innerHTML = '';
    target.innerHTML += oldTargetSegs.map(function (s) { s.class += ' oldText'; return s.outerHTML; }).join('');
    target.innerHTML += newTargetSegs.map(function (s) { s.class += ' newText'; return s.outerHTML.replace(s.innerHTML, ''); }).join('');
    /* eslint-enable no-param-reassign */
  ...
  function createTextTweens(target, newText, ops) {
    ...
    var oldTargets = [].slice.call(target.getElementsByclass('oldText')).reverse();
    var newTargets = [].slice.call(target.getElementsByclass('newText'));
