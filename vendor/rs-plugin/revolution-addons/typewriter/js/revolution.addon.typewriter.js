...
;(function() {
		...
		onbeforeswap: function(data) {
			...
			while(len--) {
				...
				if(!data.contains(itm.el)) {
					...
					itm.el.class = itm.el.class + ' hide-typewriter-cursor';
	...
	function TypewriterLayer(el, txt, levels, revapi) {
		...
		el.class = clas + ' hide-typewriter-cursor';
	...
	TypewriterLayer.prototype = {
		...
		start: function(delay, fromLoop) {
			...
			if(!fromLoop) {
				...
				this.el.innerHTML = '&nbsp;';
				this.el.class = this.el.class.replace(/ hide-typewriter-cursor/g, '');
		...
		animate: function() {
			...
			if(next) {
				...
				this.running = false;
				if(this.hideCursor) this.el.class = this.el.class + ' hide-typewriter-cursor';
		...
		*/
		restore: function(hideCursor) {
			...
			if(hideCursor) this.el.class = this.el.class + ' hide-typewriter-cursor';
