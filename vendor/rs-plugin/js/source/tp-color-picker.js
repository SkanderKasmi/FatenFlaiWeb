...
(function($) {
	...
	function newPreset(val, core, cls, grad) {
		...
		if(val !== 'blank') {
			...
			el.class = 'rev-cpicker-color blank';
			return el;
	...
	function newPoint(align, pos, val, bg) {
		...
		el.class = 'rev-cpicker-point rev-cpicker-point-' + align;
	...
	function getPoints(i) {
		...
		var rgba = RevColor.rgbValues(getAttribute(this, 'data-color'), 4),
			align = this.class.search('bottom') !== -1 ? 'bottom' : 'top',
	...
	function init() {
		...
		cPicker.on('click.revcpicker', function(e) {
			...
			if(cPicker.hasClass('open')) {
				...
				var targt = e.target,
					$this = $(targt),
					ids = targt.id,
					isTarget = targt.class.search('rev-cpicker-point') !== -1 || 
		...
		$('.rev-cpicker-point-delete').on('click.revcpicker', function() {
			...
			if(this.class.search('active') === -1) return;
